...
class database_data:
    ...
    def __init__( self, ID=0, TEMPERATURE=0, HUMIDITY=0, MOISTURE=0, PRESSURE=0, VOLTAGE=0, ON_OFF_STATE=0, ON_OFF_COMMAND=0, TEMPERATURE_POINTS=0):
        self.ID = ID
        ...
        self.ON_OFF_COMMAND = ON_OFF_COMMAND
    ...
    def save_database_data(self):
        ...
        database_data.data_table.insert().execute(DEVICE_ID=self.ID, EPOCH=time.time(), TEMPERATURE=self.TEMPERATURE, HUMIDITY=self.HUMIDITY,
                                    MOISTURE=self.MOISTURE, PRESSURE=self.PRESSURE, VOLTAGE=self.VOLTAGE, ON_OFF_COMMAND=self.ON_OFF_COMMAND,
...
def handle_database(device_id, variable_type, value):
    ...
    if(data.has_key(device_id)):
        ...
        if(variable_type == 'END'):                     
            send_message_to_device(device_id)
