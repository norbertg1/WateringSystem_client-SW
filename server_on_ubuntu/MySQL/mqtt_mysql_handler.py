...
def get_wheather_code(latitude,longitude):
	...
	weather = owm.weather_at_coords(latitude, longitude).get_weather()
	print " Weather_code:", weather.get_weather_code()
	...
	#↓↓↓↓ Innét kánikula alatt állapítom meg kell-e öntözni ↓↓↓↓
		...
		for row_hot in hottest_days_irrigation_table_select:
			if (row["DEVICE_ID"] == row_hot["DEVICE_ID"]):
				if(row_hot["IRRIGATION_ONE_TEMPERATURE"] < T and find_duplicate(row_hot["DEVICE_ID"],21) and row_hot["IRRIGATION_ONE_CHECK_TIME"].hour == datetime.datetime.now().hour ):
					print "kánikula_1"
				...
				if(row_hot["IRRIGATION_TWO_TEMPERATURE"] < T and find_duplicate(row_hot["DEVICE_ID"],22) and row_hot["IRRIGATION_TWO_CHECK_TIME"].hour == datetime.datetime.now().hour):
					print "kánikula_2"
				...
				if(row_hot["IRRIGATION_THREE_TEMPERATURE"] < T and find_duplicate(row_hot["DEVICE_ID"],23) and row_hot["IRRIGATION_THREE_CHECK_TIME"].hour == datetime.datetime.now().hour):
					print "kánikula_3"
